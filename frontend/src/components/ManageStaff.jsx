...
const ManageStaff = () => {
  ...
  const fetchStaffList = async () => {
    try {
      const res = await axios.get(
        "https://retail-edge-6kx1.onrender.com/api/staff",
      );

      // SAFE RESPONSE HANDLING
      const data = Array.isArray(res.data)
        ? res.data
        : res.data?.staff || res.data?.data || [];
  ...
  const handleActivate = async (id) => {
    try {
      await axios.patch(
        `https://retail-edge-6kx1.onrender.com/api/staff/approve/${id}`,
      );
  ...
  const handleDeactivate = async (id) => {
    try {
      await axios.patch(
        `https://retail-edge-6kx1.onrender.com/api/staff/deactivate/${id}`,
      );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `https://retail-edge-6kx1.onrender.com/api/staff/${id}`,
      );
  ...
  const filteredStaff = (Array.isArray(staffList) ? staffList : []).filter(
    (staff) =>
